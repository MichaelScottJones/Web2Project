<?php
     include_once("header.inc.php");
?>
    <script src="extrafunctions.js" type="text/javascript"></script>


    <h1 class="header">Site References</h1>
    
    <div class="readable">
        <p>This quick reference is a summary of all the regex syntax that is listed in the full reference tables, without any explanation. You can use this table if you've seen some syntax in somebody else's regex and you have no idea what feature that syntax is for. Follow the link to learn more about the syntax in the tutorial. Since the full reference tables cover a variety of regex flavors, this quick reference may have multiple entries for the same syntax with links to different sections in the tutorial if different regex flavors use the same syntax for different features.</p>

        <p>This first table includes Character Class Syntax</p>

        <code><div id="charClass"></div></code>

        <p>This second table is the complete reference guide</p>


        <code><div id="quickRef"></div></code>

        <script type="text/javascript">
            var charClass =

            {
                "Character Class Syntax": [
                    "Any character except ^-]\\",
                    "\\ (backslash) followed by any of ^-]\\",
                    "\\",
                    "- between two tokens that each specify a single character",
                    "^ immediately after the opening [",
                    "[",
                    "[",
                    "[base-[subtract]]",
                    "[base&&[intersect]]",
                    "[base&&intersect]",
                    "[:alpha:]",
                    "[:^alpha:]",
                    "\\p{Alpha}",
                    "\p{IsAlpha}",
                    "[.span-ll.]",
                    "[=x=]"
                ],

                "Feature": [
                    "Literal character",
                    "Backslash escapes a metacharacter",
                    "Literal backslash",
                    "Range",
                    "Negated character class",
                    "Literal opening bracket",
                    "Nested character class",
                    "Character class subtraction",
                    "Character class intersection",
                    "Character class intersection",
                    "POSIX class",
                    "Negated POSIX class",
                    "POSIX class",
                    "POSIX class",
                    "POSIX collation sequence",
                    "POSIX character equivalence"

                ]
            }


            var quickRef =

            {
                "Syntax": [
                    "Any character except [\\^$.|?*+()",
                    "\\ followed by any of [\\^$.|?*+(){}",
                    ".",
                    "|",
                    "\\|",
                    "?",
                    "\\?",
                    "??",
                    "?+",
                    "*",
                    "*?",
                    "*+",
                    "\\+",
                    "+?",
                    "++",
                    "{ and }",
                    "{n} where n is an integer >= 1",
                    "{n,m} where n >= 0 and m >= n",
                    "{n,} where n >= 0",
                    "{,m} where m >= 1",
                    "\\{n\\} where n is an integer >= 1",
                    "\\{n,m\\} where n >= 0 and m >= n",
                    "\\{n,\\} where n >= 0",
                    "\\{,m\\} where m >= 1",
                    "{n,m}? where n >= 0 and m >= n",
                    "{n,}? where n >= 0",
                    "{,m}? where m >= 1",
                    "{n,m}+ where n >= 0 and m >= n",
                    "{n,}+ where n >= 0",
                    "^",
                    "^",
                    "$",
                    "$",
                    "\\a",
                    "\\A",
                    "\\A",
                    "\\b",
                    "\\b",
                    "\\B",
                    "\\c",
                    "\\ca through \\cz",
                    "\\cA through \\cZ",
                    "\\C",
                    "\\B",
                    "\\d",
                    "\\D",
                    "\\e",
                    "\\f",
                    "\\g{name}",
                    "\\g-1, \\g-2, etc.",
                    "\\g{-1}, \\g{-2}, etc.",
                    "\\g1 through \\g99",
                    "\\g{1} through \\g{99}",
                    "\\g<name> where \"name\" is the name of a capturing group",
                    "\\g<name> where \"name\" is the name of a capturing group",
                    "\\g'name' where \"name\" is the name of a capturing group",
                    "\\g'name' where \"name\" is the name of a capturing group",
                    "\\g<0>",
                    "\\g'0'",
                    "\\g<1> where 1 is the number of a capturing group",
                    "\\g<1> where 1 is the number of a capturing group",
                    "\\g'1' where 1 is the number of a capturing group",
                    "\\g'1' where 1 is the number of a capturing group",
                    "\\g<-1> where -1 is a negative integer",
                    "\\g<-1> where -1 is a negative integer",
                    "\\g'-1' where -1 is is a negative integer",
                    "\\g'-1' where -1 is is a negative integer",
                    "\\g<+1> where +1 is a positive integer",
                    "\\g'+1' where +1 is is a positive integer",
                    "\\G",
                    "\\G",
                    "\\h",
                    "\\h",
                    "\\H",
                    "\\H",
                    "\\i",
                    "\\I",
                    "\\k<name>",
                    "\\k'name' through \\k'99'",
                    "\\k{name}",
                    "\\k<1> through \\k<99>",
                    "\\k'1' through \\k'99'",
                    "\\k<-1>, \\k<-2>, etc.",
                    "\\k'-1', \\k'-2', etc.",
                    "\\K",
                    "\\l",
                    "\\L",
                    "\\m",
                    "\\M",
                    "\\n",
                    "\\N",
                    "Literal CRLF, LF, or CR line break",
                    "\\o{7777} where 7777 is any octal number",
                    "\\pL where L is a Unicode category",
                    "\\PL where L is a Unicode category",
                    "\\p{L} where L is a Unicode category",
                    "\\p{IsL} where L is a Unicode category",
                    "\\p{Category}",
                    "\\p{IsCategory}",
                    "\\p{Script}",
                    "\\p{IsScript}",
                    "\\p{Block}",
                    "\\p{InBlock}",
                    "\\p{IsBlock}",
                    "\\P{Property}",
                    "\\p{^Property}",
                    "\\P{^Property}",
                    "\\Q…\\E",
                    "\\r",
                    "\\R",
                    "\\s",
                    "\\S",
                    "\\t",
                    "\\u",
                    "\\uFFFF where FFFF are 4 hexadecimal digits",
                    "\\u{FFFF} where FFFF are 1 to 4 hexadecimal digits",
                    "\\U",
                    "\\v",
                    "\\v",
                    "\\V",
                    "\\w",
                    "\\W",
                    "\\xFF where FF are 2 hexadecimal digits",
                    "\\xFFFF where FFFF are 4 hexadecimal digits",
                    "\\x{FFFF} where FFFF are 1 to 4 hexadecimal digits",
                    "\\X",
                    "\\y",
                    "\\Y",
                    "\\Z",
                    "\\0",
                    "\\1 through \\7",
                    "\\1 through \\9",
                    "\\10 through \\77",
                    "\\10 through \\99",
                    "\\100 through \\377",
                    "\\01 through \\0377",
                    "\\`",
                    "\\`",
                    "\\'",
                    "\\<",
                    "\\>",
                    "[[:<:]]",
                    "[[:>:]]"
                ],

                "Feature": [
                    "Literal character",
                    "Backslash escapes a metacharacter",
                    "Any character",
                    "Alternation",
                    "Alternation",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Lazy quantifier",
                    "Possessive quantifier",
                    "Greedy quantifier",
                    "Lazy quantifier",
                    "Possessive quantifier",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Lazy quantifier",
                    "Possessive quantifier",
                    "Literal curly braces",
                    "Fixed quantifier",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Fixed quantifier",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Greedy quantifier",
                    "Lazy quantifier",
                    "Lazy quantifier",
                    "Lazy quantifier",
                    "Possessive quantifier",
                    "Possessive quantifier",
                    "String anchor",
                    "Line anchor",
                    "Character escape",
                    "String anchor",
                    "Attempt anchor",
                    "Backspace character",
                    "Word boundary",
                    "Backslash character",
                    "XML shorthand",
                    "Control character escape",
                    "Control character escape",
                    "XML shorthand",
                    "Word boundary",
                    "Digits shorthand",
                    "Non-digits shorthand",
                    "Escape character",
                    "Form feed character",
                    "Named backreference",
                    "Relative Backreference",
                    "Relative Backreference",
                    "Backreference",
                    "Backreference",
                    "Named subroutine call",
                    "Named backreference",
                    "Named subroutine call",
                    "Named backreference",
                    "Recursion",
                    "Recursion",
                    "Subroutine call",
                    "Backreference",
                    "Subroutine call",
                    "Backreference",
                    "Relative subroutine call",
                    "Relative backreference",
                    "Relative subroutine call",
                    "Relative backreference",
                    "Forward subroutine call",
                    "Forward subroutine call",
                    "Attempt anchor",
                    "Match anchor",
                    "Hexadecimal digit shorthand",
                    "Horizontal whitespace shorthand",
                    "Non-hexadecimal digit shorthand",
                    "Non-horizontal whitespace shorthand",
                    "XML shorthand",
                    "XML shorthand",
                    "Named backreference",
                    "Named backreference",
                    "Named backreference",
                    "Backreference",
                    "Backreference",
                    "Relative Backreference",
                    "Relative Backreference",
                    "Keep text out of the regex match",
                    "Lowercase shorthand",
                    "Non-lowercase shorthand",
                    "Tcl word boundary",
                    "Tcl word boundary",
                    "Line feed character",
                    "Not a line break",
                    "Line break",
                    "Octal escape",
                    "Unicode category",
                    "Unicode category",
                    "Unicode category",
                    "Unicode category",
                    "Unicode category",
                    "Unicode category",
                    "Unicode script",
                    "Unicode script",
                    "Unicode block",
                    "Unicode block",
                    "Unicode block",
                    "Negated Unicode property",
                    "Negated Unicode property",
                    "Unicode property",
                    "Escape sequence",
                    "Carriage return character",
                    "Line break",
                    "Whitespace shorthand",
                    "Non-whitespace shorthand",
                    "Tab character",
                    "Uppercase shorthand",
                    "Unicode code point",
                    "Unicode code point",
                    "Non-uppercase shorthand",
                    "Vertical tab character",
                    "Vertical whitespace shorthand",
                    "Non-vertical whitespace shorthand",
                    "Word character shorthand",
                    "Non-word character shorthand",
                    "Hexadecimal escape",
                    "Unicode code point",
                    "Unicode code point",
                    "Unicode grapheme",
                    "Tcl word boundary",
                    "Tcl word boundary",
                    "String anchor",
                    "String anchor",
                    "NULL escape",
                    "Octal escape",
                    "Backreference",
                    "Octal escape",
                    "Backreference",
                    "Octal escape",
                    "Octal escape",
                    "String anchor",
                    "Attempt anchor",
                    "String anchor",
                    "GNU word boundary",
                    "GNU word boundary",
                    "POSIX word boundary",
                    "POSIX word boundary"
                ]
            };

            document.querySelector("#charClass").appendChild(makeTbl(charClass));
            document.querySelector("#quickRef").appendChild(makeTbl(quickRef));

        </script>
    </div>


<?php     include_once("footer.inc.php"); ?>
